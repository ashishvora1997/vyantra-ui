...
const OverviewPage: React.FC = () => (
  <div className="page-content">
    <div className="page-heading">
      ...
      <h1>Overview</h1>
      <p>
        A lightweight, fully-typed design system built with React and TypeScript. Token-based
        theming — switch between light, dark and custom themes with zero component changes.
      </p>
    </div>
      ...
      >
        {[
          ...
          >
            ...
            >
              ...
              <span
                style={{
                  ...
                  background:
                    c.status === 'stable' ? 'var(--color-success-subtle)' : 'var(--color-bg-muted)',
                  border: `1px solid ${c.status === 'stable' ? 'var(--color-success-border)' : 'var(--color-border)'}`,
                  color:
                    c.status === 'stable'
            ...
            <p style={{ fontSize: 12.5, color: 'var(--color-text-tertiary)', lineHeight: 1.5 }}>
              {c.desc}
            </p>
          </div>
        ))}
  ...
  const groups = [
    {
      ...
      rows: [
        ...
        ['--color-bg-subtle', 'Surface', 'Subtle tinted background'],
        ['--color-text-primary', 'Text', 'Primary text'],
        ['--color-primary', 'Primary', 'Brand action color'],
        ['--color-on-primary', 'Primary', 'Text on solid primary'],
        ['--color-danger', 'Danger', 'Destructive action'],
        ['--color-success', 'Success', 'Positive/confirm action'],
    ...
    },
    {
      label: 'Typography',
      rows: [
        ['--font-sans', 'Geist, system-ui', 'Sans-serif stack'],
        ['--font-mono', 'Geist Mono', 'Monospace stack'],
  ...
  return (
    <div className="page-content">
      <div className="page-heading">
        ...
        <h1>Design Tokens</h1>
        <p>
          All tokens are CSS custom properties. Components reference semantic tokens only — swap
          themes by overriding the semantic layer.
        </p>
      </div>
      {groups.map((g) => (
        <div className="section" key={g.label}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="docs-table">
              ...
              <tbody>
                {g.rows.map(([token, val, desc]) => (
                  <tr key={token}>
                    <td className="token">{token}</td>
                    <td className="type">{val}</td>
